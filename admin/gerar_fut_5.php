...
if (isset($_GET['download_all']) && $_GET['download_all'] == 1) {
    ...
    $bannerStats->recordBannerGeneration($userId, 'football', 'tema5_all', 'Banners Futebol V5 - Todos');
    
    // Gerar chave de cache para todos os banners
    $allBannersCacheKey = $bannerCache->generateCacheKey($userId, 'football_5_all', 'all', $jogos);
    ...
    $zip = new ZipArchive();
    $zipNome = "Banners_Fut_5_" . date('Y-m-d') . ".zip";
    $caminhoTempZip = sys_get_temp_dir() . '/' . uniqid('Banners_Fut_5_') . '.zip';
    $tempFiles = [];
    ...
    if ($zip->open($caminhoTempZip, ZipArchive::CREATE | ZipArchive::OVERWRITE) === TRUE) {
        foreach ($gruposDeJogos as $index => $grupoJogos) {
            ...
            $cacheKey = $bannerCache->generateCacheKey($userId, 'football_5', $index, array_intersect_key($jogos, array_flip($grupoJogos)));
            ...
            if ($cachedBanner && file_exists($cachedBanner['file_path'])) {
                ...
                if ($im) {
                    // Salvar no cache
                    $bannerCache->saveBannerToCache($userId, $cacheKey, $im, 'football_5', $index);
                    
                    $nomeArquivoTemp = sys_get_temp_dir() . '/Banners_Fut_5_' . uniqid() . '.png';
                    imagepng($im, $nomeArquivoTemp);
                    
                    $zip->addFile($nomeArquivoTemp, 'Banners_Fut_5_' . ($index + 1) . '.png');
    ...
    $cacheKey = $bannerCache->generateCacheKey($userId, 'football_5', $grupoIndex, $jogosDoGrupo);
    ...
    // Salvar no cache
    $originalName = "Banners_Fut_5_" . date('Y-m-d') . "_parte" . ($grupoIndex + 1) . ".png";
    $bannerCache->saveBannerToCache($userId, $cacheKey, $im, 'football_5', $grupoIndex, $originalName);
