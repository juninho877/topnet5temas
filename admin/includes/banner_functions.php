...
function _gerarBannerModel4($userId, $jogos, $grupoJogos, $width, $height, $padding, $heightPorJogo, $fontLiga) {
    ...
    foreach ($grupoJogos as $idx) {
        ...
        // Logo das ligas
    if ($logoLiga === null) {
        $ligas_url = 'https://i.ibb.co/C31hxmM5/Texto-do-seu-par-grafo-2.png';
        $logoLiga = @imagecreatefrompng($ligas_url);
    }
...
/**
 * Gera banner modelo 5 (gerar_fut_5.php) - Novo tema
 */
function _gerarBannerModel5($userId, $jogos, $grupoJogos, $width, $height, $padding, $heightPorJogo, $fontLiga) {
    // Criar imagem
    $im = imagecreatetruecolor($width, $height);
    $preto = imagecolorallocate($im, 0, 0, 0);
    $branco = imagecolorallocate($im, 255, 255, 255);
    ...
    $fundo = loadUserImage($userId, 'background_banner_5');
    if ($fundo) {
        imagecopyresampled($im, $fundo, 0, 0, 0, 0, $width, $height, imagesx($fundo), imagesy($fundo));
        imagedestroy($fundo);
    } else {
    ...
    static $fundoJogo = null;
    if ($fundoJogo === null) {
        $fundoJogo = loadUserImage($userId, 'card_banner_5');
        if (!$fundoJogo) {
            // Fallback para imagem padrão do primeiro script
            $fundoJogoPath = __DIR__ . '/../wtec/card/card_banner_5.png';
            $fundoJogo = file_exists($fundoJogoPath) ? imagecreatefrompng($fundoJogoPath) : false;
        }
    }
...
// Logo das ligas
        $ligas_url = 'https://i.ibb.co/bM5g6RDZ/Texto-do-seu-par-grafo.png';
        $logoL = @imagecreatefrompng($ligas_url);

    
    if ($logoL) {
        imagecopy($im, $logoL, 0, 1560, 0, 0, imagesx($logoL), imagesy($logoL));
    }
    ...
    $logoUsuario = loadUserImage($userId, 'logo_banner_5');
    if ($logoUsuario) {
        $w = imagesx($logoUsuario); $h = imagesy($logoUsuario);
        if ($w > 0 && $h > 0) {
